...
export function describeCollateral(spec: CollateralSpec): string {
  if (spec.kind === "any") return "Any Boy";
  if (spec.tokenIds.length === 1) return `Boy #${spec.tokenIds[0]}`;
  return `${spec.tokenIds.length} specific Boys`;
}
